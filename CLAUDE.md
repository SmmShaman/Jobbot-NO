...
│   ├── add_user_id_to_system_logs.sql  # Multi-user data isolation (NEW!)
...
│   └── add_telegram_link_code.sql      # Telegram link code for multi-user
...
**system_logs**
- `id`, `user_id` (for multi-user isolation)
...
- Cost tracking per operation per user
...
- **Problem**: New users could see admin's data:
  ...
  - AI cost statistics ($X.XX)
  - Job counts from other users
  - CV profiles from other users
- **Root cause**: API functions queried tables without `user_id` filter
- **Solution**: Added `user_id` filtering throughout the system

**Database changes** (`database/add_user_id_to_system_logs.sql`):
- Added `user_id` column to `system_logs` table
- Updated RLS policies for user isolation
- Added index for faster user-specific queries

**API changes** (`services/api.ts`):
- `getJobs()` - Now filters by current user's `user_id`
- `getTotalCost()` - Now sums only current user's costs
- `getSystemLogs()` - Now returns only current user's logs
- `cv.getProfiles()` - Now returns only current user's profiles
- `cv.getActiveProfile()` - Now returns only current user's active profile

**Edge Function changes** (`supabase/functions/scheduled-scanner/index.ts`):
- Now inserts per-user system_logs with `user_id` field
- Each user gets their own scan log for cost tracking

**Types changes** (`types.ts`):
- Added `user_id?: string` to `SystemLog` interface

**Verification**:
1. Log in as new user → Dashboard shows $0.00 cost, 0 jobs
2. Activity log shows only that user's actions
